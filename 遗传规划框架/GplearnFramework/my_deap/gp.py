...
#    License along with DEAP. If not, see <http://www.gnu../licenses/>.
    ...
    def from_string(cls, string, pset):
        ...
        for token in tokens:
            ...
            if token in pset.mapping:
                ...
                if type_ is not None and not (primitive.ret == type_):  # modified here
...
class ConstTerminal(object):
    """Class that encapsulates terminal primitive in expression. Terminals can
    be values or 0-arity functions.
    """
    __slots__ = ('name', 'value', 'ret', 'conv_fct', 'const_range')

    def __init__(self, terminal, const_range, symbolic, ret):
        self.ret = ret
        self.value = terminal
        self.name = str(terminal)
...
class PrimitiveSetTyped(object):
        ...
        def addType(dict_, ret_type):
            if ret_type not in dict_:
                ...
                for type_, list_ in dict_.items():
                    ...
                    if type_ == ret_type: # modified here
                        for item in list_:
    ...
    def addTerminal(self, terminal, ret_type, symbolic = False, name=None):
    ...
    def addConstTerminal(self, terminal, const_range, ret_type, symbolic = False, name=None):
        """Add a terminal to the set. Terminals can be named
        using the optional *name* argument. This should be
        used : to define named constant (i.e.: pi); to speed the
        evaluation time when the object is long to build; when
        ...
        prim = ConstTerminal(terminal, const_range, symbolic, ret_type)
        self._add(prim)
        self.terms_count += 1
    ...
    def addEphemeralConstant(self, name, ephemeral, ret_type):
        ...
        if name not in self.module_gp:
            class_ = type(name, (Ephemeral,), {'func': staticmethod(ephemeral),
                                               'ret': ret_type})
            self.module_gp[name] = class_
        else:
            class_ = self.module_gp[name]
...
class My_PrimitiveSet(PrimitiveSetTyped):
    """Class same as :class:`~deap.gp.PrimitiveSetTyped`, except there is no
    definition of type.
    """

    def __init__(self, name, arity, input_type, prefix="ARG"):
        args = [input_type] * arity
        PrimitiveSetTyped.__init__(self, name, args, __type__, prefix)

    def addPrimitive(self, primitive, arity, args, name=None):
        """Add primitive *primitive* with arity *arity* to the set.
        If a name *name* is provided, it will replace the attribute __name__
        attribute to represent/identify the primitive.
        """
        assert arity > 0, "arity should be >= 1"
        # args = [__type__] * arityac
        PrimitiveSetTyped.addPrimitive(self, primitive, args, __type__, name)

    def addTerminal(self, terminal, input_type, symbolic, name=None):
        """Add a terminal to the set."""
        PrimitiveSetTyped.addTerminal(self, terminal, input_type, symbolic, name)

    def addConstTerminal(self, terminal, const_range, input_type, symbolic, name=None):
        """Add a terminal to the set."""
        PrimitiveSetTyped.addConstTerminal(self, terminal, const_range, input_type, symbolic, name)

    def addEphemeralConstant(self, name, ephemeral, input_type):
        """Add an ephemeral constant to the set."""
        PrimitiveSetTyped.addEphemeralConstant(self, name, ephemeral, input_type)
...
def generate(pset, min_, max_, condition, type_=None):
    ...
    while len(stack) != 0:
        ...
        if condition(height, depth) or (type_[:5]=='const'):  # modified here
            try:
            ...
            # modified here
            if type_[:5]=='const':
                term = Terminal(str(random.choice(term.const_range)), True, type_)
            # if isclass(term):
            #     term = term()
            expr.append(term)
        else:
...
def mutNodeReplacement(individual, pset):
    ...
    if node.arity == 0:  # Terminal
        ...
        # modified
        if term.ret[:5] == 'const':
            term = Terminal(str(random.choice(term.const_range)), True, term.ret)
        # if isclass(term):
        #     term = term()
